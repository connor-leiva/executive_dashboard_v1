 ...
 * revenue.
 *
 * Served inside the tenant bundle at /platform rather than from its own host. That is a
 * deliberate trade: admin.<PLATFORM_DOMAIN> has no DNS record today, and a console nobody can
 * reach is worth less than one behind a separate origin. Nothing here is a security boundary —
 * the operator API requires a `pu` token that no tenant session can produce — so the cost of
 * co-hosting is that the login page is visible, not that anything is reachable.
 */
...
const card = { background: T.white, border: `1px solid ${T.line}`, borderRadius: 12, padding: 20 };
const label = {
...
const input = {
  ...
  border: `1px solid ${T.line}`, fontFamily: FONT, fontSize: 14, color: T.ink,
  background: T.white, outline: "none",
};

function Button({ children, onClick, kind = "primary", disabled, type = "button" }) {
  const kinds = {
    primary: { background: T.evergreen, color: T.onDark, border: `1px solid ${T.evergreen}` },
    quiet: { background: T.white, color: T.slate, border: `1px solid ${T.line}` },
    danger: { background: T.white, color: T.poppyText, border: `1px solid ${alpha(T.poppy, 0.5)}` },
  };
  return (
...
function Notice({ error, children }) {
  ...
  return (
    <div style={{
      background: error ? alpha(T.poppy, 0.09) : T.meadowBg,
      border: `1px solid ${error ? alpha(T.poppy, 0.35) : T.sprout}`,
      color: error ? T.poppyText : T.meadowInk,
...
function InviteLink({ url }) {
  ...
  return (
    <div style={{ marginTop: 12 }}>
      ...
      <div style={{ fontFamily: FONT, fontSize: 12, color: T.tertiary, marginTop: 6 }}>
...
function StatusDot({ status, errors, syncFailures }) {
  ...
  const dot = bad ? T.poppy : warn ? T.daffodil : T.meadow;
  const text = bad ? "Suspended" : warn ? "Needs attention" : "Healthy";
  return (
    <span style={{ display: "inline-flex", alignItems: "center", gap: 6,
                   fontFamily: FONT, fontSize: 12.5, color: T.tertiary }}>
...
function Stat({ k, v }) {
  return (
    <div>
      ...
      <div style={{ fontFamily: FONT, fontSize: 14, color: T.ink, marginTop: 5,
...
function OperatorLogin({ onDone }) {
  ...
  return (
    <div style={{ minHeight: "100vh", background: T.parchment, display: "flex",
                  alignItems: "center", justifyContent: "center", padding: 20 }}>
      <form onSubmit={submit} style={{ ...card, width: 380, maxWidth: "100%" }}>
        <div style={{ fontFamily: HEAD, fontSize: 19, fontWeight: 600, color: T.ink }}>
          Operator console
        </div>
        <div style={{ fontFamily: FONT, fontSize: 13, color: T.tertiary, margin: "6px 0 18px" }}>
  ...
  if (result) {
    return (
      <div style={{ ...card, marginBottom: 20 }}>
        <div style={{ fontFamily: HEAD, fontSize: 16, fontWeight: 600, color: T.ink }}>
          {result.slug} is live
        </div>
        <div style={{ fontFamily: FONT, fontSize: 13, color: T.tertiary, marginTop: 6 }}>
    ...
    <form onSubmit={submit} style={{ ...card, marginBottom: 20 }}>
      ...
      <div style={{ display: "grid", gap: 12,
        ...
        <div>
          ...
          <div style={{ fontFamily: FONT, fontSize: 12, color: T.tertiary, marginTop: 5 }}>
...
function WorkspaceDetail({ slug, onBack, onChanged }) {
  ...
  if (!row) {
    return (
      <div style={card}>
        <Notice error>{err}</Notice>
        <div style={{ fontFamily: FONT, fontSize: 13, color: T.tertiary }}>
          {err ? "" : "Loading…"}
        </div>
  ...
  return (
    <div>
      <button onClick={onBack} style={{
        ...
        fontFamily: FONT, fontSize: 13, fontWeight: 600, color: T.slate,
      ...
      <div style={{ ...card, marginBottom: 16 }}>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", gap: 16,
                      flexWrap: "wrap" }}>
          <div>
            <div style={{ fontFamily: HEAD, fontSize: 20, fontWeight: 600, color: T.ink }}>
              {row.name}
            </div>
            <div style={{ fontFamily: FONT, fontSize: 13, color: T.tertiary, marginTop: 4 }}>
              {row.slug} ·{" "}
          ...
          <div style={{ marginTop: 18 }}>
            ...
            {row.errors.map((e, i) => (
              <div key={i} style={{
                ...
                background: alpha(T.poppy, 0.07), border: `1px solid ${alpha(T.poppy, 0.28)}`,
                fontFamily: FONT, fontSize: 12.5, color: T.ink,
              }}>
      ...
      <div style={card}>
        <div style={{ fontFamily: HEAD, fontSize: 15, fontWeight: 600, color: T.ink,
                      marginBottom: 4 }}>
          Recent activity
        </div>
        <div style={{ fontFamily: FONT, fontSize: 12.5, color: T.tertiary, marginBottom: 12 }}>
          Who was invited, what was connected, who signed in. Operational history, not their data.
        </div>
        {events.length === 0 && (
          <div style={{ fontFamily: FONT, fontSize: 13, color: T.muted }}>Nothing recorded yet.</div>
        )}
        {events.map((e, i) => (
          <div key={i} style={{
            display: "flex", justifyContent: "space-between", gap: 12, padding: "8px 0",
            borderTop: i ? `1px solid ${T.line}` : "none", fontFamily: FONT, fontSize: 13,
          }}>
            <span style={{ color: T.ink }}>{e.action}</span>
            <span style={{ color: T.muted, whiteSpace: "nowrap" }}>{relativeTime(e.at) || ""}</span>
          </div>
        ))}
      </div>
    </div>
    ...
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center",
        ...
        <div>
          ...
          <div style={{ fontFamily: FONT, fontSize: 13, color: T.tertiary, marginTop: 3 }}>
      ...
      <div style={{ display: "grid", gap: 12 }}>
        {(rows || []).map((r) => (
          <button key={r.slug} onClick={() => onOpen(r.slug)} style={{
            ...
            <div>
              <div style={{ fontFamily: HEAD, fontSize: 15.5, fontWeight: 600, color: T.ink }}>
                {r.name}
              </div>
              <div style={{ fontFamily: FONT, fontSize: 12.5, color: T.tertiary, marginTop: 4 }}>
            ...
            <div style={{ textAlign: "right" }}>
              ...
              <div style={{ fontFamily: FONT, fontSize: 12, color: T.muted, marginTop: 4 }}>
...
export default function PlatformConsole() {
  ...
  return (
    <div style={{ minHeight: "100vh", background: T.parchment, fontFamily: FONT }}>
      <div style={{
        display: "flex", justifyContent: "space-between", alignItems: "center",
        padding: "14px 22px", background: T.evergreen, gap: 12, flexWrap: "wrap",
      }}>
        <span style={{ fontFamily: HEAD, fontSize: 15, fontWeight: 600, color: T.onDark }}>
          Acumyn · Operator console
        </span>
        <span style={{ display: "flex", alignItems: "center", gap: 14 }}>
          <span style={{ fontSize: 12.5, color: T.onDarkMute }}>{opName()}</span>
          <button onClick={() => { clearOpSession(); setAuthed(false); }} style={{
            background: "none", border: `1px solid ${alpha(T.onDark, 0.35)}`, borderRadius: 7,
            padding: "5px 11px", color: T.onDark, fontFamily: FONT, fontSize: 12.5,

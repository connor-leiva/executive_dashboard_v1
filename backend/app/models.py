...
class RecruitingAction(Base):
    """The outbox. One row per write ATTEMPT against a customer's GoHighLevel (§4.6, §5.3).

    EVERY WRITE GOES THROUGH HERE, and the row is committed BEFORE the call is made. That order
    is the whole point: if the process dies mid-send there is a record that something was
    attempted, which is the difference between "we do not know" and "nothing happened". A text to
    a recruit cannot be un-sent, so the system has to be able to say which of those it is.

    `idempotency_key` is generated by the CLIENT, once per click. A double-click, a retried
    request or a refresh all carry the same key and collapse to one row -- there is no GHL-side
    idempotency for a message, so this table is the only thing standing between an impatient
    double-click and two texts to the same person.

    A row stuck in `sending` for more than two minutes is NOT auto-retried. It goes to `failed`
    with "unknown outcome, check GHL before resending", because a duplicate text to a recruit is
    worse than a missing one and only a person can tell which happened.

    `request` keeps the exact body we sent, including the message text, for audit -- purged at
    365 days by the outbox tick, in line with RECALL_TRANSCRIPT_RETAIN_DAYS. It never contains the
    token: the redaction happens on the way in, not on the way out.
    """
    __tablename__ = "recruiting_action"
    id: Mapped[uuid.UUID] = mapped_column(GUID(), primary_key=True, default=_uuid)
    tenant_id: Mapped[uuid.UUID] = mapped_column(ForeignKey("tenant.id", ondelete="CASCADE"), index=True)
    ...
    candidate_id: Mapped[uuid.UUID | None] = mapped_column(ForeignKey("recruiting_candidate.id", ondelete="SET NULL"), nullable=True, index=True)
    queue_item_id: Mapped[uuid.UUID | None] = mapped_column(ForeignKey("recruiting_queue_item.id", ondelete="SET NULL"), nullable=True)
    # send_sms | send_email | add_note | upsert_task | complete_task | move_stage | book | log_call
    kind: Mapped[str] = mapped_column(String(24), index=True)
    request: Mapped[dict] = mapped_column(JSONType, default=dict)
    # queued | sending | sent | failed | refused | dry_run
    status: Mapped[str] = mapped_column(String(12), default="queued", index=True)
    attempts: Mapped[int] = mapped_column(Integer, default=0, server_default="0")
    next_attempt_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True, index=True)
    ghl_ref: Mapped[str | None] = mapped_column(String(64), nullable=True)
    http_status: Mapped[int | None] = mapped_column(Integer, nullable=True)
    # What a PERSON should be told. GHL's own message is kept for owners only (§5.7); this is the
    # sentence the drawer shows, already mapped.
    error: Mapped[str | None] = mapped_column(String(400), nullable=True)
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now(), index=True)
    sent_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True)
    __table_args__ = (
        # Per TENANT, not globally: two workspaces generating the same uuid is impossible in
        # practice and catastrophic in principle, and scoping it costs nothing.
